...
import type { ProviderProps } from '@/components/CodeBlock/playground/Provider'
import Provider from '@/components/CodeBlock/playground/Provider'
...
const FencedCodeBlock: React.FC<FencedCodeBlockProps> = (props) => {
  ...
  const highlightedLines = useMemo(() => {
    ...
    return highlights.split(',').reduce<number[]>((acc, cur) => {
      ...
      const rows = []
      if (limits.length === 1) {
        rows.push(Number.parseInt(limits[0]))
      }
      else if (limits.length === 2) {
        const start = Number.parseInt(limits[0])
